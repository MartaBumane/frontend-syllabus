...
function allInfoAboutAllMovies(){
  ...
  for(let i = 0; i<ratingArray.length; i++){ 
      for(let j = 0;j<averageRatings.length;j++){
          ...
          if(averageRatings[j].name===ratingArray[i].name){
            count++;
            averageRatings[j].rating = (averageRatings[j].rating+ratingArray[i].rating);
            averageRatings[j].ratingCount = count;                   
          }    
      }
      let filterEqualNames = averageRatings.filter(word =>word.name === ratingArray[i].name);
      if(filterEqualNames.length<1){
        averageRatings.push({name: ratingArray[i].name, rating: ratingArray[i].rating, ratingCount : 1});
      }
    }
  ...
  for(let x=0; x<averageRatings.length;x++){
    ...
    let average = Math.round((averageRatings[x].rating/averageRatings[x].ratingCount) * 100)/100;
    average = average.toFixed(1);
    console.log("Movie name: " + averageRatings[x].name+ ", average rating: "+average+ ", "+ averageRatings[x].ratingCount+ " people rated this movie!" + availability);
...
async function fillVideoStore() {
  ...
  while (addMore) {
    const { movieName, movieRating, more } = await inquirer.prompt([
      ...
      { type: "number", name: "movieRating", message: "Enter movie rating from 1 to 10" },
